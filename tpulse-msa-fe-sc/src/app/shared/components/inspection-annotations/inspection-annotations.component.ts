...
@Component({
  selector: 'app-inspection-annotations',
  templateUrl: './inspection-annotations.component.html',
  styleUrls: ['./inspection-annotations.component.scss']
})
export class InspectionAnnotationsComponent implements OnInit {
  ...
  @Output() emitAnnotation = new EventEmitter();
  annotationMode: boolean = true;
  ...
  ngOnChanges() {
    if (this.newComment === '' && this.newReply === '') {
      ...
      if (this.imageModalData?.annotations?.length > 0) {
  ...
  onSave() {
    ...
    let el: any = document.getElementById('modalImage');
    let data = { 'image': el.src, 'color': this.canvasData.line_color, 'drawing_type': annotation.shape, 'thickness': this.canvasData.thickness, 'label': this.canvasData.comments, 'coordinates': coordinates };
    this.emitAnnotation.emit(data);
    this.notify = "Annotation saved successfully"
    var x: any = document.getElementById('showNotification');
    x.className = 'show';
    setTimeout(function () {
