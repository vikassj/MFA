...
export class CommonService {
  ...
  readConfigurationsData() {
    ...
    return this.http.get(assetUrl('/json/' + fileName)).pipe(map(
      (data: any) => {
        ...
        console.log("dfgggg : " + this.defaultModule)
        return data;
      }
    ));
  }
  ...
  postUserActivityData(userActivityData: any) {
    let url = sessionStorage.getItem('apiUrl') + 'api/' + sessionStorage.getItem('selectedPlant') + '/safety_and_surveillance/user_clicks/';
  ...
  //   let url = sessionStorage.getItem('apiUrl') + 'api/' + sessionStorage.getItem('selectedPlant')  +   '/safety_and_surveillance/user_click_view/';
  //   return this.http.post(url, data).pipe(map(
  //     data => {
  //       return data;
  //     }
